...
SCREEN_WIDTH = 800
SCREEN_HEIGHT = 800
reduction_amount = 0.001
pheromone_increase = 0.1
pheromone_start = 0.1
num_ants = 100
num_food_sources = 50  # Anzahl der Nahrungsquellen
MIN_PHEROMONE = 0.001
...
food_sources = {node: random.randint(1, 100) for node in end_nodes}  # Futterquellen mit zufälliger Größe
...
def draw_graph(screen, adjacency_list, num_rows, num_columns, width, height, ants, food_sources):
    for node, data in adjacency_list.items():
        ...
        for neighbor, n_data in data['neighbors'].items():
            ...
            intensity = math.log(n_data['pheromone'] + 1) / math.log(pheromone_start + 1)  # Logarithmische Skalierung
            edge_color = (255, max(0, min(255, int(255 * (1 - intensity)))), max(0, min(255, int(255 * (1 - intensity)))))
    ...
    for goal_node, size in food_sources.items():
        ...
        blue_intensity = math.log(size + 1) / math.log(101)  # Logarithmische Skalierung
        goal_color = (max(0, min(255, int(255 * (1 - blue_intensity)))), max(0, min(255, int(255 * (1 - blue_intensity)))), 255)
...
class Ant:
    ...
    def move(self, graph):
        ...
        # Shortest Path will be utilised in 60% of the cases 
        if self.returning and p_shortest_path <= 0.60:
...
ants = [Ant(start_node, end_nodes, food_sources) for _ in range(num_ants)]
